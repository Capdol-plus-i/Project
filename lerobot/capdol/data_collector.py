...
class RobotDataCollector:
    ...
    def __init__(self, robot, output_dir="collected_data"):
        ...
        self.camera_id = 1  # 카메라 ID를 8로 설정
        self.width = 640
        self.height = 480
        self.running = False
    ...
    def _main_loop(self):
        ...
        try:
            while self.running:
                ...
                # 화면 표시
                cv2.imshow('Robot Data Collection', frame)
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='Robot and Hand Data Collector')
    ...
    parser.add_argument('--camera', type=int, default=8,
                        help='Camera ID to use (default: 8)')
    ...
    try:
        ...
        collector = RobotDataCollector(robot, args.output)
        collector.camera_id = args.camera  # 명령행에서 지정한 카메라 ID 설정
        collector.start()
        ...
        print("  s - Take a single sample (snapshot)")
        print("  q - Quit the program")
        print("\nMove the leader arm manually to collect data.")
