...
class HandRobotController:
    ...
    def start_camera(self):
        ...
        logger.info(f"Camera opened with ID {self.camera_id}")
        return True
    ...
    def process_frame(self):
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks, handedness in zip(
                ...
                cv2.putText(
                    frame,
                    f"Predicted: {self.predicted_joints.round().astype(int)}",
                    (10, 90),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (0, 255, 0),
        ...
        try:
            while self.running:
                ...
                if frame is not None:
                    cv2.imshow('Hand Controlled Robot', frame)
            ...
            logger.error(f"Error in hand control loop: {e}", exc_info=True)
        finally:
            self.stop()
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='Hand Controlled Robot')
    ...
    try:
        ...
        controller = HandRobotController(robot, args.model, args.arm)
        controller.start()
        
        # 메인 스레드는 여기서 대기
        print("\n=== Hand Controlled Robot ===")
        print("Move your hand in front of the camera to control the robot")
