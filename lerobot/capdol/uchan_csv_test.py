...
print("Controls:")
print("'s' key: Save snapshot to CSV")
print("'q' key or ESC: Quit program")
print("Make sure the OpenCV window is focused to receive keyboard input")
...
with mp_hands.Hands(
  ...
  while cap_1.isOpened() and cap_2.isOpened():
    ...
    # Initialize variables for snapshot data
    ...
    if results_1.multi_hand_landmarks and results_1.multi_handedness and results_2.multi_hand_landmarks and results_2.multi_handedness:
      for cam1_hand_landmarks, cam1_handedness, cam2_hand_landmarks, cam2_handedness in zip(results_1.multi_hand_landmarks, results_1.multi_handedness, results_2.multi_hand_landmarks, results_2.multi_handedness):
        ...
        tip_zx = int(cam2_index_finger_tip.x * width)
        ...
        # Draw detection
        cv2.circle(image_1, (tip_x, tip_y), 10, (0, 0, 255), -1)
        cv2.putText(image_1, f"Hand{hand_type}", (tip_x-30, tip_y-20), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        
        cv2.circle(image_2, (tip_zx, tip_z), 10, (0, 0, 255), -1)
        cv2.putText(image_2, f"Hand{hand_type}", (tip_zx-30, tip_z-20), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    ...
    cv2.imshow('MediaPipe Hands 2', cv2.flip(image_2, 1))
    ...
    if key == ord('q') or key == 27:  # 'q' key or ESC key
        ...
        if save_snapshot_to_csv(data):
            print(f"Snapshot saved to CSV: tip_x={tip_x}, tip_y={tip_y}, tip_z={tip_z}, joints={pred}")
        else:
            print("Failed to save snapshot to CSV")
