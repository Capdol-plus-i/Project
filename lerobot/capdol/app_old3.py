...
3. 두 개의 카메라로 손의 검지 끝 좌표 추적
4. leader arm과 follower arm의 모터 위치와 두 카메라의 검지 끝 좌표를 동시에 기록
5. 수집된 데이터를 CSV 파일로 저장
6. 웹 브라우저를 통해 두 카메라 피드 확인 및 제어 가능
"""
...
# 각도를 모터 스텝으로 변환하는 함수 (convert_degrees_to_steps 대체)
def degrees_to_steps(degrees):
    """각도를 모터 스텝으로 변환"""
    return (np.array(degrees) * STEPS_PER_DEGREE).astype(np.int32)
...
class DualCameraDataCollector:
    ...
    def __init__(self, robot, output_dir="collected_data"):
        ...
        self.camera2_id = 0  # 두 번째 카메라 ID 기본값
        self.width = 640
        self.height = 480
        self.running = False
    ...
    def process_frames(self):
        ...
        if leader_positions is not None:
            leader_step_positions = leader_positions  # 이미 스텝 단위라면 변환 필요 없음
        else:
            leader_step_positions = None
        ...
        if follower_positions is not None:
            follower_step_positions = follower_positions  # 이미 스텝 단위라면 변환 필요 없음
        else:
            follower_step_positions = None
    ...
    def take_snapshot(self):
        ...
        try:
            ...
            filename = os.path.join(self.output_dir, f"robot_hand_data_snapshot_{timestamp_str}.csv")
            ...
            with open(filename, 'w', newline='') as csv_file:
                ...
                # follower arm 위치 읽기 (leader 위치 복사 후)
                self.robot.copy_leader_to_follower()
                follower_positions = self.robot.get_positions('follower')
                if follower_positions is not None:
                    follower_step_positions = follower_positions  # 이미 스텝 단위라면 변환 필요 없음
...
def create_app(output_dir='lerobot/capdol/collected_data', camera1_id=1, camera2_id=2):
    ...
    try:
        ...
        # 사용 설명
        print("\n=== Robot Data Collection Web Interface (Dual Camera) ===")
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='Robot and Hand Data Collector Web Interface (Dual Camera)')
