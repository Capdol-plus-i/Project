#!/usr/bin/env python3
"""
로봇 암 모터 위치 데이터 수집 프로그램 - follower arm 수동 제어 버전

이 스크립트는 다음을 수행합니다:
1. follower arm의 토크를 해제하여 사용자가 직접 움직일 수 있게 함
2. 두 개의 카메라로 손의 검지 끝 좌표 추적 (선택적)
3. follower arm의 모터 위치를 기록
4. 수집된 데이터를 CSV 파일로 저장
5. 웹 브라우저를 통해 두 카메라 피드 확인 및 제어 가능
"""
...
class ManipulatorRobot:
    def __init__(self, config):
        ...
        self.calibration_loaded = False  # 항상 False로 유지
    ...
    def setup_for_data_collection(self):
        """데이터 수집을 위한 설정: follower arm 토크 해제 설정"""
        if not self.is_connected:
            logger.error("Robot is not connected. Cannot setup for data collection.")
            return False
        
        try:
            # follower arm 토크 해제 (사용자가 직접 움직일 수 있게)
            for name in self.follower_arms:
                logger.info(f"Disabling torque on {name} follower arm for manual movement")
                self.follower_arms[name].write("Torque_Enable", self.TorqueMode.DISABLED.value)
            
            logger.info("Robot setup for data collection completed: follower arm torque disabled")
            return True
    ...
    def get_positions(self, arm_type='follower'):
        ...
        Args:
            arm_type: 읽을 팔 유형 ('follower'만 지원)
        ...
        if arm_type != 'follower':
            logger.error("Only follower arm is supported")
            return None
...
class DualCameraDataCollector:
    """
    로봇 관절 위치와 두 카메라의 손 좌표를 동시에 기록하는 데이터 수집기
    """
    def __init__(self, robot, output_dir="collected_data"):
        ...
        self.camera1_id = 4  # 첫 번째 카메라 ID 기본값
        self.camera2_id = 1  # 두 번째 카메라 ID 기본값
        self.width = 640
        self.height = 480
        self.running = False
    ...
    def start_recording(self):
        ...
        try:
            ...
            filename = os.path.join(self.output_dir, f"follower_arm_data_{timestamp}.csv")
            
            # CSV 파일 열기
            self.csv_file = open(filename, 'w', newline='')
            self.csv_writer = csv.writer(self.csv_file)
            
            # 헤더 작성 (follower arm 모터 위치만 포함)
            header = [
                "frame", "timestamp", 
    ...
    def process_frames(self):
        ...
        follower_step_positions = convert_degrees_to_steps(follower_positions) if follower_positions is not None else None
    ...
    def process_hand_detection(self, frame, camera_idx):
        ...
        # 캘리브레이션 상태 표시 (항상 NOT CALIBRATED)
        calibration_status = "NOT CALIBRATED"
        self.status_info["calibrated"] = False
        cv2.putText(
            frame,
            calibration_status,
            (10, self.height - 10),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 0, 255),  # 빨간색 (캘리브레이션 안 됨)
            2
        )
        
        # 토크 상태 표시
        torque_status = "TORQUE DISABLED"
        cv2.putText(
            frame,
            torque_status,
            (10, self.height - 40),
    ...
    def take_snapshot(self):
        """단일 샘플 기록 (스냅샷) - follower arm 모터 위치만 기록"""
        if self.recording:
        ...
        try:
            ...
            filename = os.path.join(self.output_dir, f"follower_arm_snapshot_{timestamp_str}.csv")
...
def create_app(output_dir='lerobot/capdol/collected_data', camera1_id=1, camera2_id=2):
    """Flask 앱 및 로봇 초기화"""
    ...
    try:
        ...
        # 사용 설명
        print("\n=== 로봇 Follower Arm 데이터 수집 인터페이스 ===")
        print(f"- 데이터 저장 위치: {output_dir}")
        print(f"- 카메라 1 ID: {camera1_id}")
        print(f"- 카메라 2 ID: {camera2_id}")
        print("\n웹 브라우저에서 http://localhost:5000 주소로 접속하여 데이터 수집을 제어할 수 있습니다.")
        print("Follower arm을 직접 손으로 움직여 데이터를 수집하세요.")
        print("토크가 해제되어 팔을 자유롭게 움직일 수 있습니다.")
        print("캘리브레이션을 수행하지 않고 모터 값을 기록합니다.\n")
