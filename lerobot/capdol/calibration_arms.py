...
def compute_nearest_rounded_position(position, models):
    ...
    delta_turn = convert_degrees_to_steps(ROTATED_POSITION_DEGREE)
...
class ArmCalibrator:
    ...
    def connect_robot(self, arm_type=None):
        ...
        Args:
            arm_type: 연결할 팔 유형 ('leader', 'follower', None=모두)
        """
        logger.info("로봇 초기화 중...")
        self.config = KochRobotConfig()
        
        if arm_type != 'follower':  # leader 또는 둘 다
        ...
        if arm_type != 'leader':  # follower 또는 둘 다
    ...
    def calibrate_arm(self, arm_type):
        ...
        Args:
            arm_type: 캘리브레이션할 팔 유형 ('leader' 또는 'follower')
        """
    ...
    def _calibrate_single_arm(self, arm, arm_type, arm_name):
        ...
        zero_target_pos = convert_degrees_to_steps(ZERO_POSITION_DEGREE)
        ...
        rotated_target_pos = convert_degrees_to_steps(ROTATED_POSITION_DEGREE)
...
def main():
    ...
    parser.add_argument('--arm', type=str, default=None, choices=['leader', 'follower'],
                        help='Arm to calibrate (leader or follower)')
