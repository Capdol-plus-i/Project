...
# 음성 인식 설정
RATE = 16000
CHUNK = int(RATE / 20)  # 50ms 단위 캡처
WAKE_WORDS = ["하이봇", "하이못", "아이봇", "AI봇", "5", "아이", "하이"]
CMD_MAP = {
    "왼쪽": ["왼쪽", "왼 쪽", "왼"],
    "오른쪽": ["오른쪽", "오른 쪽", "오른"],
    "위": ["위", "위로", "위쪽"],
            ...
            for response in responses:
                ...
                if any(wake in transcript for wake in WAKE_WORDS):
                    if result.is_final or result.stability > 0.8:
                        ...
                        time.sleep(0.3)  # 약간의 대기 후 새 세션 시작

                        # 명령어 모드
                        command_stream, command_responses = start_stream()
