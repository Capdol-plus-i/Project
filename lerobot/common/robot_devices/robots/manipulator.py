...
class ManipulatorRobot:
    ...
    def connect(self):
        ...
        if self.config.gripper_open_degree is not None:
            ...
            for name in self.leader_arms:
                # gripper가 있는 경우에만 실행
                if "gripper" in self.leader_arms[name].motors:
    ...
    def set_koch_robot_preset(self):
        def set_operating_mode_(arm):
            ...
            all_motors = arm.motor_names
            all_motors_except_gripper = [name for name in all_motors if name != "gripper"]
            if len(all_motors_except_gripper) > 0:
                # 4 corresponds to Extended Position on Koch motors
                arm.write("Operating_Mode", 4, all_motors_except_gripper)

            # Configure gripper only if it exists
            if "gripper" in arm.motors:
        ...
        for name in self.follower_arms:
            ...
            # Set better PID values to close the gap between recorded states and actions
            if "elbow_flex" in self.follower_arms[name].motors:
                self.follower_arms[name].write("Position_P_Gain", 1500, "elbow_flex")
                self.follower_arms[name].write("Position_I_Gain", 0, "elbow_flex")
                self.follower_arms[name].write("Position_D_Gain", 600, "elbow_flex")
        ...
        if self.config.gripper_open_degree is not None:
            for name in self.leader_arms:
                ...
                # Enable torque on the gripper of the leader arms only if gripper exists
                if "gripper" in self.leader_arms[name].motors:
                    self.leader_arms[name].write("Torque_Enable", 1, "gripper")
                    self.leader_arms[name].write("Goal_Position", self.config.gripper_open_degree, "gripper")
    ...
    def send_action(self, action: torch.Tensor, arm_type='follower') -> torch.Tensor:
        """Command the robot arms to move to a target joint configuration.
        ...
        Args:
            action: tensor containing the concatenated goal positions for the arms.
            arm_type: string specifying which arms to control ('follower' or 'leader').
        
        Returns:
            torch.Tensor: The actual action sent to the robot after any clipping.
        ...
        # Select the appropriate arms based on arm_type
        ...
        for name in arms:
            # Get goal position of each arm by splitting the action vector
            motor_names = arms[name].motor_names
            to_idx += len(motor_names)
            goal_pos = action[from_idx:to_idx]
            from_idx = to_idx

            # Cap goal position when too far away from present position.
            # Slower fps expected due to reading from the arm.
            if self.config.max_relative_target is not None:
                present_pos = arms[name].read("Present_Position")
                present_pos = torch.from_numpy(present_pos)
                goal_pos = ensure_safe_goal_position(goal_pos, present_pos, self.config.max_relative_target)
            ...
            # Send goal position to each arm
            goal_pos = goal_pos.numpy().astype(np.float32)
